...
def year_format(year: str, Date: object):
    ...
    if len(num) == 1:
        ...
        if search_custom(year, ["đầu"]):
            # Đầu năm 2022
            if num[0] == Date.year:
                new_date = datetime(year=num[0], month=Date.month, day=Date.day, hour=Date.hour, minute=Date.minute) 
                new_date += relativedelta(days=1)
            # Đầu năm 2023 (2023 > Date.year)
            else:
                new_date = datetime(year=num[0], month=1, day=1, hour=Date.hour, minute=Date.minute) 

        elif search_custom(year, ["cuối"]):
            # Cuối năm 2022
            new_date = datetime(year=num[0], month=12, day=31, hour=Date.hour, minute=Date.minute)
        
        elif search_custom(year, ["nữa"]):
            # 1 năm nữa
            new_date = datetime(year=Date.year, month=Date.month, day=Date.day, hour=Date.hour, minute=Date.minute) 
        ...
        if search_custom(year, ["đầu"]):
            ...
            if search_custom(year, ["sau nữa", "tới nữa"]):
                ...
                new_date = datetime(year=Date.year, month=1, day=1, hour=Date.hour, minute=Date.minute)
            ...
            if search_custom(year, ["sau nữa", "tới nữa"]):
                ...
                new_date = datetime(year=Date.year, month=12, day=31, hour=Date.hour, minute=Date.minute)
                new_date += relativedelta(years=2)
...
def month_format(month: str, Date: object):
    ...
    if len(num) == 1:
        ...
        if search_custom(month, ["đầu"]):
            # Đầu tháng 2
            if num[0] == Date.month:
                new_date = datetime(year=Date.year, month=num[0], day=Date.day, hour=Date.hour, minute=Date.minute) 
                new_date += relativedelta(days=1)
            # Đầu tháng 3 (3 > Date.month)
            else:
                new_date = datetime(year=Date.year, month=num[0], day=1, hour=Date.hour, minute=Date.minute)
        ...
        if search_custom(month, ["đầu"]):
            ...
            if search_custom(month, ["sau nữa", "tới nữa"]):
                ...
                new_date = datetime(year=Date.year, month=Date.month, day=1, hour=Date.hour, minute=Date.minute)
...
def time_format(time: str, Date: object):
    num = [int(num) for num in time.split() if num.isdigit()] # get number in string
    # 1h chiều -> 13h
    # 10h tối  -> 22h
    if num != []: 
    ...
    if len(num) == 1:
        if search_custom(time, ["rưỡi"]):
            if search_custom(time, ["nữa"]):
                ...
                # 10 rưỡi -> 10:30
                new_date = datetime(year=Date.year, month=Date.month, day=Date.day, hour=num[0], minute=30)
            ...
            # 10h
            new_date = datetime(year=Date.year, month=Date.month, day=Date.day, hour=num[0])
        ...
        if search_custom(time, ["kém", "thiếu"]):
            ...
            # 10h30
            new_date = datetime(year=Date.year, month=Date.month, day=Date.day, hour=num[0], minute=num[1])
    else:
        if search_custom(time, ["trưa"]):
            # Trưa nay - Sau giờ trưa - Đầu giờ trưa
            new_date = datetime(year=Date.year, month=Date.month, day=Date.day, hour=13)

        elif search_custom(time, ["chiều", "tối", "tan", "làm", "ca"]):
            # Chiều nay - Tối nay - Tan làm - Tan ca - Sau giờ làm - Sau ca làm việc
            new_date = datetime(year=Date.year, month=Date.month, day=Date.day, hour=18)

        elif search_custom(time, ["chút", "chốc", "tí", "xíu", "hồi", "lát", "nữa"]):
            new_date = datetime(year=Date.year, month=Date.month, day=Date.day, hour=Date.hour)
            # + 1 hours
